...
export function CountdownStrip() {
  ...
  return (
    <section aria-label="Countdown to Summer Build Camp" className="relative z-20 px-4 pb-10 sm:px-6 lg:pb-14">
        ...
        <div className="relative overflow-hidden rounded-[26px] border-2 border-[#132d63] bg-[#f5c843] px-4 py-5 shadow-[0_8px_0_#132d63] sm:px-7">
          ...
          {phase === "countdown" ? <div className="relative flex flex-col items-center gap-5 lg:flex-row lg:justify-between lg:gap-10">
            <div className="shrink-0 text-center lg:text-left">
              <h2 className="sbc-display text-2xl font-black uppercase leading-none text-[#132d63] sm:text-3xl lg:text-[2.1rem]">Ready. Set.<br className="hidden lg:block"/> <span className="text-[#168c91]">Innovate!</span></h2>
            </div>
          ...
          </div> : phase === "live" ? <div className="relative flex items-center justify-center gap-4 py-2 text-center"><span className="h-4 w-4 animate-pulse rounded-full bg-[#168c91] ring-8 ring-[#168c91]/20"/><h2 className="sbc-display text-4xl font-black uppercase leading-none text-[#132d63] sm:text-6xl">SBC is live!</h2></div> : <div className="relative flex flex-col items-center justify-between gap-4 py-1 text-center sm:flex-row sm:text-left"><h2 className="sbc-display text-3xl font-black uppercase leading-none text-[#132d63] sm:text-5xl">Ready for the next build?</h2><Link href={path("/register")} className="rounded-2xl bg-[#132d63] px-6 py-3 font-black text-white shadow-[0_5px_0_#168c91] transition hover:-translate-y-1">Join the next cohort</Link></div>}
        </div>
      </div>
    </section>
  )
