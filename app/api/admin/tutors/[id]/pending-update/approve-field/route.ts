...
export async function POST(
  ...
  try {
    ...
    const fieldValue = pendingChanges[field];
    ...
    // If there are remaining changes, update pending_changes
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { error: 'Failed to apply change', details: updateError.message },
        { status: 500 }
      );
    }
    ...
    try {
      const notificationMessage = hasRemainingFields
        ? `"${fieldName}" approved. ${Object.keys(remainingChanges).length} update(s) still pending review.`
        : `"${fieldName}" approved. All updates are now live on your profile.`;
        ...
        body: JSON.stringify({
          ...
          message: notificationMessage,
            metadata: {
              ...
              approved_field_names: fieldName,
              remaining_fields: Object.keys(remainingChanges),
              all_approved: !hasRemainingFields,
            },
            priority: 'high',
            sendEmail: false,
          }),
    ...
    return NextResponse.json({ 
      success: true,
      message: `Field "${fieldName}" approved and applied successfully`,
