...
export async function POST(request: NextRequest) {
  try {
    ...
    } else if (externalId.startsWith('payout_')) {
      await handlePayoutStatus({
        supabase,
        transactionId: transId,
        status: normalizedStatus,
        payoutRequestId: externalId.replace('payout_', ''),
        failureReason: failureReason || undefined,
      });
    } else {
...
async function handlePayoutStatus({
  ...
  try {
    console.log(`💰 Processing payout status for request: ${payoutRequestId}, status: ${status}`);
    ...
    if (status === 'SUCCESS') {
      ...
      console.log(`✅ Payout completed: ${payoutRequestId}, amount: ${amount} XAF`);
      ...
      await supabase
        ...
        .insert({
          ...
          message: `Your payout of ${amount.toFixed(0)} XAF has been successfully processed and sent to your mobile money account.`,
          data: {
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          status: 'failed',
          admin_notes: failureReason || 'Payout failed on Fapshi',
          updated_at: new Date().toISOString(),
        })
        .eq('id', payoutRequestId);
      ...
      // Revert earnings status back to 'active' so tutor can request payout again
      await supabase
        .from('tutor_earnings')
        .update({
          earnings_status: 'active',
      ...
      console.log(`❌ Payout failed: ${payoutRequestId}, reason: ${failureReason || 'Unknown'}`);
      ...
      await supabase
        ...
        .insert({
          ...
          type: 'payout_failed',
          title: 'Payout Failed',
          message: `Your payout request of ${amount.toFixed(0)} XAF could not be processed. ${failureReason ? `Reason: ${failureReason}` : 'Please try again or contact support.'}`,
          data: {
            payout_request_id: payoutRequestId,
            amount: amount,
            status: 'failed',
      ...
      // PENDING or other status - just update transaction ID if not set
      if (!payoutRequest.fapshi_trans_id) {
        await supabase
          .from('payout_requests')
          .update({
            fapshi_trans_id: transactionId,
    ...
    console.error('❌ Error handling payout status webhook:', error);
    throw error;
  }
}
