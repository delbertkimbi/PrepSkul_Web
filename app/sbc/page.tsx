...
import { SBC_CURRICULUM_WEEKS, SBC_PACKAGES, SBC_SCHEDULE } from "@/lib/sbc/content"
  ...
  return (
    <SbcPageShell>
      ...
      <main>
        ...
        <section id="registration" className="px-4 py-20 sm:px-6 lg:py-28"><div className="mx-auto max-w-5xl"><PaperSheet tone="yellow" className="overflow-hidden p-7 sm:p-12" rotate={1}><Tape color="cream" className="-top-3 right-16 rotate-6"/><div className="grid items-center gap-8 md:grid-cols-[1fr_auto]"><div><Eyebrow className="bg-white">Registration is open</Eyebrow><h2 className="sbc-display mt-5 text-4xl font-black uppercase sm:text-6xl">Save their seat.</h2><p className="mt-4 max-w-xl leading-7 text-slate-700">{t("Choose the level of mentorship and support that fits your child. Every package includes the complete five-day camp.")}</p><p className="mt-4 text-xl font-black">{t("Packages from")} {SBC_PACKAGES[0].price.toLocaleString()} XAF <span className="text-sm font-semibold text-slate-600">{t("per child")}</span></p><Link href={path("/pricing")} className="mt-2 inline-block text-sm font-bold text-[#168c91] underline decoration-[#132d63] decoration-2 underline-offset-4">{t("Compare packages")}</Link></div><Link href={path("/register")}><PaperButton className="w-full md:w-auto">{t("Start registration")} <ArrowRight className="ml-2 h-5 w-5"/></PaperButton></Link></div></PaperSheet></div></section>
      </main>
      <SbcFooter />
    </SbcPageShell>
  )
