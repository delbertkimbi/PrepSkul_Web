    ...
    <SbcPageShell>
      ...
      <main>
          ...
          <div className="mx-auto grid max-w-6xl items-center gap-10 lg:grid-cols-[1.02fr_.98fr]">
            <div className="relative z-10 text-center lg:text-left">
              ...
              <h1 className="sbc-display mt-5 text-[2.55rem] font-black uppercase leading-[.9] text-[#132d63] min-[375px]:text-[2.9rem] sm:text-7xl lg:text-[5.6rem]">
                <span className="text-[#2864d7]">1 Week</span><span className="block">AI &amp; Innovation</span><span className="block text-[#168c91]">Experience</span>
              </h1>
              <p className="mt-6 text-lg font-bold text-[#132d63] sm:text-xl"><span className="text-[#2864d7]">Learn.</span> <span className="text-[#6a47bd]">Build.</span> <span className="text-[#e06b2d]">Innovate.</span> <span className="text-[#168c91]">Pitch.</span></p>
              ...
              <div className="mt-7 flex flex-col justify-center gap-4 sm:flex-row lg:justify-start">
                <Link href={path("/register")}><PaperButton className="w-full sm:w-auto">Register now <ArrowRight className="ml-2 h-5 w-5" /></PaperButton></Link>
        ...
        <section id="roadmap" className="px-4 py-20 sm:px-6 lg:py-28">
          <div className="mx-auto max-w-6xl"><ScrollReveal className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><Eyebrow>5-day roadmap</Eyebrow><h2 className="sbc-display mt-4 text-4xl font-black uppercase sm:text-6xl">One week to <span className="text-[#2864d7]">create</span>, innovate and pitch.</h2></div><Link href={path("/program")} className="shrink-0 font-black text-[#2864d7] underline decoration-[#f5c843] decoration-4 underline-offset-4">Explore each day →</Link></ScrollReveal>
        ...
        <section id="build" className="bg-[#132d63] px-4 py-20 text-white sm:px-6 lg:py-24"><div className="mx-auto max-w-6xl"><div className="grid items-center gap-10 lg:grid-cols-2"><ScrollReveal><Eyebrow>What they’ll build</Eyebrow><h2 className="sbc-display mt-5 text-4xl font-black uppercase sm:text-6xl">Come with ideas.<br/><span className="text-[#f5c843]">Leave with proof.</span></h2><p className="mt-5 max-w-lg leading-7 text-white/70">Every learner finishes with something tangible they can demonstrate, explain and continue improving.</p></ScrollReveal><div className="grid grid-cols-2 gap-4">{builds.map(({icon:Icon,title,color},i)=><PaperSheet key={title} className="p-5 text-[#132d63]" rotate={i%2?2:-2}><Icon className={`h-8 w-8 ${color}`}/><h3 className="mt-4 font-black uppercase leading-tight">{title}</h3></PaperSheet>)}</div></div></div></section>
