-- Migration: Add missing columns to ticha_presentations table
-- Run this in Supabase SQL Editor if you get "Could not find the 'presentation_data' column" error
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'ticha_presentations' 
    AND column_name = 'presentation_data'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'ticha_presentations' 
    AND column_name = 'refinement_history'
  ) THEN
    ALTER TABLE ticha_presentations 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'ticha_presentations' 
    AND column_name = 'design_preset'
  ) THEN
    ALTER TABLE ticha_presentations 
    ADD COLUMN design_preset TEXT;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'ticha_presentations' 
    AND column_name = 'design_customizations'
  ) THEN
    ALTER TABLE ticha_presentations 
    ADD COLUMN design_customizations JSONB;
  END IF;
END $$;
...
-- Create design templates table if it doesn't exist
...
-- Create design inspiration table if it doesn't exist
